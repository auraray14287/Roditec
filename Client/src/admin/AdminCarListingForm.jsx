...
const STYLES = `
  ...
  .clf-section {
    ...
    text-transform:uppercase; letter-spacing:.5px; color:#3B6BF0;
    grid-column:1/-1; padding-bottom:8px; border-bottom:1px solid #E5E7F0;
    margin-top:8px; display:flex; align-items:center; gap:6px;
  }
  .clf-section::before { content:''; width:3px; height:12px; background:#3B6BF0; border-radius:2px; }
  .clf-submit {
    width:100%; margin-top:20px; background:#3B6BF0; color:#fff; border:none;
    padding:13px; border-radius:8px; font-family:'Inter',sans-serif;
    font-weight:600; font-size:14px; cursor:pointer; transition:all .2s;
  }
  .clf-submit:hover:not(:disabled) { background:#2952CC; transform:translateY(-1px); box-shadow:0 4px 14px rgba(59,107,240,.3); }
...
const AdminCarListingForm = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.make || !formData.model || !formData.year || !formData.price || !formData.RentSell) {
      alert('Please fill in Make, Model, Year, Price and Rent/Sell fields.');
      return;
    }

    // The backend requireListingAuth middleware checks for admin-id header
    const adminId = localStorage.getItem('adminId');
    if (!adminId) {
      alert('Admin session not found. Please log out and log in again.');
      return;
    }

    const data = new FormData();
    Object.keys(formData).forEach((key) => {
      const skipKeys = ['images', 'certified'];
      if (!skipKeys.includes(key)) data.append(key, formData[key]);
    });
    if (selectedFile) {
    ...
    try {
      ...
      const msg = error.response?.data?.message || 'Failed to add listing. Please try again.';
      alert(msg);
    } finally {
      setSubmitting(false);
    }
    ...
    <div className="clf-root">
      ...
      <form onSubmit={handleSubmit} encType="multipart/form-data">
        <div className="clf-grid">
          ...
          <div className="clf-field">
            <label className="clf-label">User ID <span style={{textTransform:'none',fontWeight:400,color:'#AAAACC'}}>(optional)</span></label>
            <input className="clf-input" type="text" name="user_id" value={formData.user_id} onChange={handleInputChange} placeholder="Link to a user account"/>
          </div>
          ...
          <div className="clf-field">
            <label className="clf-label">VIN <span style={{textTransform:'none',fontWeight:400,color:'#AAAACC'}}>(optional)</span></label>
        ...
        <button type="submit" className="clf-submit" disabled={submitting}>
